...
class Icegram {
    ...
    function __construct() {
        ...
        if( is_admin() && current_user_can( 'edit_posts' ) ) {
            ...
            add_action( 'admin_init', array( &$this, 'ig_dismiss_admin_notice' ) );
    ...
    public function ig_add_admin_notices_star() { 
        ...
        //show if one or more campaigns are published or more than 1 in draft
	...
	public static function ig_submit_survey() {
		...
		if ( ! empty( $_POST ) ) {
			...
			if ( ! empty( $_POST['btn-val'] ) ) {
				...
				if ( 'cancel' === $_POST['btn-val'] ) {
					update_option( 'ig_survey_for_problems_cancelled', true );
					exit();
				} elseif ( 'no' === $_POST['btn-val'] ) {
					unset( $_POST['ig_email'] );
				}
			}
			...
			if ( wp_remote_retrieve_response_code( $response ) == 200 ) {
				...
				if ( empty( $data['error'] ) ) {
					if ( ! empty( $data ) && ! empty( $data['success'] ) ) {
						update_option( 'ig__survey_for_problems_done', true );
					}
	...
	public static function ig_dismiss_admin_notice() {
